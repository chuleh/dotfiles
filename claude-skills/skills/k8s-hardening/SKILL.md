...
- See `docker-hardening` skill for image content rules.
...
- See `terraform-standards` skill for IaC-side rules.
...
- `values.yaml` defaults are **production-safe** (see Variable Defaults policy in `terraform-standards`): non-root, drops caps, read-only FS, resource limits set, replicas ≥ 2 for HA workloads.
